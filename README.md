...
After the gate passes, the command creates a plan and asks for approval. After approval, it creates a reusable Agent Skill under `.agents/skills/<goal-id>/SKILL.md`, records human-readable goal context under `.opencode/goals/<goal-id>/goal.md`, records structured runtime state under `.opencode/goals/<goal-id>/state.json`, executes the workflow, persists subagent handoffs under `.opencode/goals/<goal-id>/handoffs/`, then runs an isolated `verification-agent` pass based only on those handoffs. The `.agents/skills` file is the canonical skill; `state.json` is the plugin runtime source of truth for active, complete, and blocked status.

If verification fails, the workflow runs one correction pass from the verifier handoff notes, records new handoffs, and verifies once more. The command marks the goal complete only when verifier evidence proves the acceptance criteria, or blocked only when the same blocker is documented in handoffs.

The plugin exposes `goal_get_status`, `goal_list_goals`, and `goal_update_status` tools for structured lifecycle tracking. New handoffs write `#achieved`; legacy handoffs containing the old `#acheieved` heading are still accepted when read.
